...
def parse_args():
    parser = argparse.ArgumentParser(
        description="no_interaction_loss-SwitchTransformer"
    )
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/no_interaction_loss/switchgate/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "no_interaction_loss-Transformer",
        "lr": args.lr,
    ...
    if len(seeds) == 1:
        ...
        val_acc, val_f1, val_auc, test_acc, test_f1, test_auc = (
            train_and_evaluate_no_interaction_loss(
                args, args.seed, fusion_model, "switchgate"
            )
        )
        ...
        for seed in seeds:
            ...
            val_acc, val_f1, val_auc, test_acc, test_f1, test_auc = (
                train_and_evaluate_no_interaction_loss(
                    args, seed, fusion_model, "switchgate"
                )
            )
            val_accs.append(val_acc)
            val_f1s.append(val_f1)
