...
from src.ablation.train_no_interaction_loss import (
    train_and_evaluate_no_interaction_loss,
)


# Parse input arguments
def parse_args():
    parser = argparse.ArgumentParser(description="no_interaction_loss-moepp")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/no-intearction-loss/moepp/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    model_kwargs = {
        "model": "no-interaction-loss-moepp",
