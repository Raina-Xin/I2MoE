...
from src.ablation.train_less_perturbed_forward import (
    train_and_evaluate_less_perturbed_forward,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="less_perturbed_forward-moepp")
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/less_perturbed_forward/moepp/{args.data}",
        f"{args.data}",
        f"{args.modality}.txt",
    )
    ...
    if len(seeds) == 1:
        fusion_model = MoEPlusPlusTransformer(
            num_modalities,
            args.num_patches,
            args.hidden_dim,
            n_labels,
        ...
        val_acc, val_f1, val_auc, test_acc, test_f1, test_auc = (
            train_and_evaluate_less_perturbed_forward(
                args, args.seed, fusion_model, "moepp"
            )
        )
