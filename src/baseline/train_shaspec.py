...
from src.common.fusion_models.shaspec import ShaSpec
...
def parse_args():
    parser = argparse.ArgumentParser(description="shaspec")
    parser.add_argument("--device", type=int, default=0)
    parser.add_argument("--data", type=str, default="adni")
    parser.add_argument("--initial-filling", type=str, default="mean")
    parser.add_argument(
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/baseline/shaspec/{args.data}", f"{args.data}", f"{args.modality}.txt"
    )
