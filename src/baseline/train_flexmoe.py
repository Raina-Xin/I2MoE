...
from src.common.fusion_models.flexmoe import FlexMoE
...
def get_supporting_group(
    missing_modality=None, observed_modalities=None, mc_idx_dict=None, num_samples=2
):
    ...
    available_mcs = [
        mc for mc in all_set if contains_all_chars(sufficient_modalities, mc)
    ]
...
def train_and_evaluate(args, seed):
    ...
    if args.data == "adni" or args.data == "mosi":
        criterion = torch.nn.CrossEntropyLoss()
    elif args.data == "mimic":
        criterion = torch.nn.CrossEntropyLoss(torch.tensor([0.25, 0.75]).to(device))
    elif args.data == "enrico":
        criterion = torch.nn.CrossEntropyLoss()  # CCA_objective(16)
    elif args.data == "mosi_regression":
    ...
    for epoch in trange(args.train_epochs):
        ...
        mc_idx_dict_train = {
            k: list(set(v) & set(train_ids)) for k, v in mc_idx_dict.items()
        }
        mc_idx_dict_valid = {
            k: list(set(v) & set(train_ids + valid_ids)) for k, v in mc_idx_dict.items()
        }
        mc_idx_dict_test = {
            k: list(set(v) & set(train_ids + test_ids)) for k, v in mc_idx_dict.items()
        }
...
def main():
    args, _ = parse_args()
    logger = setup_logger(
        f"./logs/baseline/flexmoe/{args.data}", f"{args.data}", f"{args.modality}.txt"
    )
